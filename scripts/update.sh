#!/usr/bin/env bash
#
# claude-persona update script
# https://github.com/kellyredding/claude-persona
#
# This script is fetched and executed by `claude-persona update`.
# It can be modified without releasing new CLI versions.
#
set -euo pipefail

# Configuration
REPO="kellyredding/claude-persona"
BINARY_NAME="claude-persona"
BACKUP_DIR="${HOME}/.claude-persona/update-backup"
...
Update script: https://raw.githubusercontent.com/${REPO}/main/scripts/update.sh
EOF
}
...
main() {
  ...
  if [[ -z "$install_path" ]]; then
    echo -e "${RED}claude-persona not found in PATH${NC}" >&2
    exit 1
  fi
  ...
  local tarball_name="claude-persona-${latest_version}-${platform}.tar.gz"
  local checksum_name="claude-persona-${latest_version}-${platform}.tar.gz.sha256"
  local download_url="${GITHUB_RELEASES}/v${latest_version}/${tarball_name}"
  local checksum_url="${GITHUB_RELEASES}/v${latest_version}/${checksum_name}"
  local script_url="https://raw.githubusercontent.com/${REPO}/main/scripts/update.sh"
  ...
  if [[ "$PREVIEW" == true ]]; then
    ...
    if [[ "$current_version" == "$latest_version" ]]; then
      ...
      echo "    claude-persona update force"
    else
  ...
  if ! check_permissions "$install_path"; then
    ...
    echo "    sudo claude-persona update" >&2
    echo "" >&2
    echo "  Or reinstall to a user-writable location:" >&2
    echo "    ~/.local/bin/claude-persona" >&2
    exit 1
  fi
  ...
  echo -e "${CYAN}${action_word} claude-persona${NC}"
  echo ""
  echo "  Current version:  ${current_version}"
  ...
  # Binary is named claude-persona-{version}-{platform} in the tarball
  ...
  echo ""
  echo "  Verify: claude-persona version"
}
